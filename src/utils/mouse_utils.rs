...
pub fn mouse_world_coords(
    ...
    return Vector2 {
        x: mouse_screen_pos_scaled_with_render_target.x - game_context.camera.offset.x + game_context.camera.target.x,
        y: mouse_screen_pos_scaled_with_render_target.y - game_context.camera.offset.y + game_context.camera.target.y,
    };
